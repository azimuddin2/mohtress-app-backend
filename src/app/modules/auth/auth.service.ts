...
const forgotPassword = async (payload: TForgotPassword) => {
  ...
  let query: any = {};
  if (isEmail) {
    query.email = phoneOrEmail.trim().toLowerCase();
  } else {
    query.phone = phoneOrEmail.trim();
  }
  ...
  // 4️⃣ email or phone OTP sending
  if (isEmail) {
    ...
    return { identifier: user.email, userId: user._id, accessToken };
  } else {
    // Twilio OTP
    ...
    return { identifier: user.phone, userId: user._id, accessToken };
  }
};
